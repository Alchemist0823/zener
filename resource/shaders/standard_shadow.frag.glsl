...
void main(){
	...
	if ( texture2D(depthMap, shadowCoord.xy ).z  <  shadowCoord.z){
    	visibility = 0.5;
	}
	
	colorLight = vec4(phongModel(position, normal, texColor.rgb), 1.0) * visibility;
    colorOut = colorLight * MultipleColor + AddColor;//vec4(mix(m_Color.rgb, texColor.rgb, texColor.a), 1.0);
}