...
public abstract class GameContainer {
	...
	public static int getBuildVersion() {
		try {
			...
			props.load(ResourceManager.getInstance().getResourceAsStream("version"));
			
			int build = Integer.parseInt(props.getProperty("build"));
			Log.info("Zener Build #"+build);
			
			return build;
		} catch (Exception e) {
			Log.error("Unable to determine Zener build number");
			return -1;
		}
	}
	...
	protected void updateAndRender(int delta) throws SlickException {
		...
		if (clearEachFrame) {
			glClear(SGL.GL_COLOR_BUFFER_BIT | SGL.GL_DEPTH_BUFFER_BIT);
		}
