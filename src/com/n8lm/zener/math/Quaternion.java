...
//        float n = (float) Math.invSqrt(norm());
//        x *= n;
//        y *= n;
//        z *= n;
//        w *= n;
//    }
    ...
    public void lookAt(Vector3f direction, Vector3f up) {
        ...
        Vector3f vect3 = new Vector3f();
        Vector3f vect1 = new Vector3f();
        Vector3f vect2 = new Vector3f();
		vect3.set(direction).normalizeLocal();
        vect1.set(up).crossLocal(direction).normalizeLocal();
        vect2.set(direction).crossLocal(vect1).normalizeLocal();
        fromAxes(vect1, vect2, vect3);
    }
    
    public void lookAt(Vector3f source, Vector3f dest, Vector3f up) {

    	Vector3f normalDest = dest.normalize();
    	
    	float dot = source.dot(normalDest);

        if (Math.abs(dot - (-1.0f)) < 0.000001f)
        {
            // vector a and b point exactly in the opposite direction, 
            // so it is a 180 degrees turn around the up-axis
            fromAngleAxis((float) Math.toRadians(180.0f), up);
        }
        if (Math.abs(dot - (1.0f)) < 0.000001f)
        {
