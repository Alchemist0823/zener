...
public final class Matrix4f implements Cloneable, java.io.Serializable {
    ...
    public void fromFrame(Vector3f location, Vector3f direction, Vector3f up, Vector3f left) {
        ...
        Vector3f f = new Vector3f().set(direction);
        Vector3f s = new Vector3f().set(f).crossLocal(up);
        Vector3f u = new Vector3f().set(s).crossLocal(f);
//        s.normalizeLocal();
//        u.normalizeLocal();
...
//        set(transMatrix);
     ...
     * position. If the position is invalid a Exception is
     * thrown.
     * 
     * @param i
     *            the row index.
...
//        }
        ...
        float[] matrixWrite = new float[16];


        fillFloatArray(matrixWrite, columnMajor);
        fb.put(matrixWrite, 0, 16);
...
?	*/
    ...
    public void angleRotation(Vector3f angles) {
        ...
        angle = (angles.z * (float) (Math.PI / 180.0));
        sy = (float) Math.sin(angle);
        cy = (float) Math.cos(angle);
        angle = (angles.y * (float) (Math.PI / 180.0));
        sp = (float) Math.sin(angle);
        cp = (float) Math.cos(angle);
        angle = (angles.x * (float) (Math.PI / 180.0));
    ...
    public void setInverseRotationRadians(float[] angles) {
        ...
        double cr = (float) Math.cos(angles[0]);
        double sr = (float) Math.sin(angles[0]);
        double cp = (float) Math.cos(angles[1]);
        double sp = (float) Math.sin(angles[1]);
        double cy = (float) Math.cos(angles[2]);
        double sy = (float) Math.sin(angles[2]);
    ...
    public void setInverseRotationDegrees(float[] angles) {
        ...
        vec[0] = (angles[0] * (float) (Math.PI / 180.0));
        vec[1] = (angles[1] * (float) (Math.PI / 180.0));
        vec[2] = (angles[2] * (float) (Math.PI / 180.0));
