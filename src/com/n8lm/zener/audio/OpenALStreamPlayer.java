...
public class OpenALStreamPlayer {
	...
	private void initStreams() throws IOException {
		...
		if (url != null) {
			...
			audio = new OggInputStream(ResourceManager.getInstance().getResourceAsStream(ref));
		}
		...
		try {
			...
			if (count != -1) {
				...
				try {
					...
					LOGGER.log(Level.SEVERE, "Failed to loop buffer: "+bufferId+" "+format+" "+count+" "+audio.getRate(), e);
					return false;
				}
			} else {
				if (loop) {
