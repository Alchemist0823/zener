...
public class SoundStore {
	...
	private final static Logger LOGGER = Logger.getLogger(OpenALStreamPlayer.class
		      .getName());
	...
	public void init() {
		...
		AccessController.doPrivileged(new PrivilegedAction() {
            public Object run() {
				try {
					...
					LOGGER.log(Level.SEVERE, "Sound initialisation failure.", e);
					soundWorks = false;
					sounds = false;
					music = false;
				}
		...
		if (soundWorks) {
			...
			if (AL10.alGetError() != AL10.AL_NO_ERROR) {
				...
				music = false;
				soundWorks = false;
				LOGGER.log(Level.SEVERE, "- AL init failed");
			} else {
	...
	public Audio getMOD(String ref) throws IOException {
		return getMOD(ref, ResourceManager.getInstance().getResourceAsStream(ref));
	}
	...
	public Audio getAIF(String ref) throws IOException {
		return getAIF(ref, ResourceManager.getInstance().getResourceAsStream(ref));
	}
	...
	public Audio getWAV(String ref) throws IOException {
		return getWAV(ref, ResourceManager.getInstance().getResourceAsStream(ref));
	}
	...
	public Audio getOgg(String ref) throws IOException {
		return getOgg(ref, ResourceManager.getInstance().getResourceAsStream(ref));
	}
		...
		if (music) {
			if (mod != null) {
				try {
					...
					LOGGER.log(Level.SEVERE, "Error with OpenGL MOD Player on this this platform", e);
					mod = null;
				}
			}
			if (stream != null) {
				try {
					...
					LOGGER.log(Level.SEVERE, "Error with OpenGL Streaming Player on this this platform", e);
					mod = null;
				}
			}
		}
