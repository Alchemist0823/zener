...
public class OggInputStream extends InputStream implements AudioInputStream {
	...
	private final static Logger LOGGER = Logger.getLogger(OggInputStream.class
		      .getName());
	...
	/**
	 * @see com.n8lm.zener.audio.AudioInputStream#getChannels()
	 */
	...
	/**
	 * @see com.n8lm.zener.audio.AudioInputStream#getRate()
	 */
	public int getRate() {
		return oggInfo.rate;
	}
	...
	private boolean getPageAndPacket() {
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "Failure reading in vorbis", e);
			endOfStream = true;
			return false;
		}
		...
		if (syncState.pageout(page) != 1) {
			...
			LOGGER.severe("Input does not appear to be an Ogg bitstream.");
			endOfStream = true;
			return false;
		}
		...
		if (streamState.pagein(page) < 0) {
			...
			LOGGER.severe("Error reading first page of Ogg bitstream data.");
			endOfStream = true;
			return false;
		}
		...
		if (streamState.packetout(packet) != 1) {
			...
			LOGGER.severe("Error reading initial header packet.");
			endOfStream = true;
			return false;
		}
		...
		if (oggInfo.synthesis_headerin(comment, packet) < 0) {
			...
			LOGGER.severe("This Ogg bitstream does not contain Vorbis audio data.");
			endOfStream = true;
			return false;
		}
		...
		while (i < 2) {
				...
				if (result == 1) {
					...
					while (i < 2) {
						...
						if (result == -1) {
							...
							LOGGER.severe("Corrupt secondary header.  Exiting.");
							endOfStream = true;
							return false;
						}
			...
			try {
				...
				LOGGER.log(Level.SEVERE, "Failed to read Vorbis: ", e);
				endOfStream = true;
				return false;
			}
			if (bytes == 0 && i < 2) {
				LOGGER.severe("End of file before finding all Vorbis headers!");
				endOfStream = true;
				return false;
			}
			syncState.wrote(bytes);
			...
			while (!endOfBitStream) {
				while (!endOfBitStream) {
					...
					if (result == -1) { // missing or corrupt data at this page position
						LOGGER.severe("Corrupt or missing data in bitstream; continuing...");
					} else {
							...
							if (result == -1) { // missing or corrupt data at this page position
								...
								while ((samples = dspState.synthesis_pcmout(_pcm,
									...
									if (bytesToWrite >= pcmBuffer.remaining()) {
										LOGGER.warning("Read block from OGG that was too big to be buffered: " + bytesToWrite);
									} else {
				...
				if (!endOfBitStream) {
					...
					if (index >= 0) {
						...
						try {
							...
							LOGGER.log(Level.SEVERE, "Failure during vorbis decoding", e);
							endOfStream = true;
							return;
						}
					} else {
	...
	/**
	 * @see com.n8lm.zener.audio.AudioInputStream#atEnd()
	 */
