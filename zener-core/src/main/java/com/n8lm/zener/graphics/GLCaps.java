...
/**
 * Created by forrestsun on 7/20/15.
 */

import com.n8lm.zener.glsl.ShaderSource;

import java.util.Collection;

/**
 * <code>GLCaps</code> is an enum specifying a capability that the {@link Renderer}
 * supports.
 *
...
public enum GLCaps {
     ...
     *
     * @param caps The collection of renderer capabilities {@link Renderer#getGLCaps() }.
     * @param tex The texture to check
     * @return True if it is supported, false otherwise.
     */
    /*public static boolean supports(Collection<GLCaps> caps, Texture tex){
        if (tex.getType() == Texture.Type.TwoDimensionalArray
                && !caps.contains(GLCaps.TextureArray))
            return false;
        ...
        switch (fmt){
            case Depth24Stencil8:
                return caps.contains(GLCaps.PackedDepthStencilBuffer);
            case Depth32F:
                return caps.contains(GLCaps.FloatDepthBuffer);
            case RGB16F_to_RGB111110F:
            case RGB111110F:
                return caps.contains(GLCaps.PackedFloatTexture);
            case RGB16F_to_RGB9E5:
            case RGB9E5:
                return caps.contains(GLCaps.SharedExponentTexture);
            default:
                if (fmt.isFloatingPont())
                    return caps.contains(GLCaps.FloatTexture);

                return true;
        }
    }

    private static boolean supportsColorBuffer(Collection<GLCaps> caps, RenderBuffer colorBuf){
        ...
        switch (colorFmt){
            case RGB111110F:
                return caps.contains(GLCaps.PackedFloatColorBuffer);
            case RGB16F_to_RGB111110F:
            case RGB16F_to_RGB9E5:
            case RGB9E5:
                return false;
            default:
                if (colorFmt.isFloatingPont())
                    return caps.contains(GLCaps.FloatColorBuffer);
     ...
     * @return True if it is supported, false otherwise.
     */
    /*public static boolean supports(Collection<GLCaps> caps, FrameBuffer fb){
        if (!caps.contains(GLCaps.FrameBuffer))
            return false;

        if (fb.getSamples() > 1
                && !caps.contains(GLCaps.FrameBufferMultisample))
            return false;
        ...
        if (depthBuf != null){
            ...
            if (!depthFmt.isDepthFormat()){
                ...
                if (depthFmt == Format.Depth32F
                        && !caps.contains(GLCaps.FloatDepthBuffer))
                    return false;

                if (depthFmt == Format.Depth24Stencil8
                        && !caps.contains(GLCaps.PackedDepthStencilBuffer))
                    return false;
            }
        }
    ...
    */
     ...
     * @return True if it is supported, false otherwise.
     */
    public static boolean supports(Collection<GLCaps> caps, ShaderSource shader){
        if (shader.getSource().startsWith("GLSL")) {
            int ver = Integer.parseInt(shader.getSource().substring(4));
            switch (ver) {
                case 100:
                    if (!caps.contains(GLCaps.GLSL100)) return false;
                case 110:
                    if (!caps.contains(GLCaps.GLSL110)) return false;
                case 120:
                    if (!caps.contains(GLCaps.GLSL120)) return false;
                case 130:
                    if (!caps.contains(GLCaps.GLSL130)) return false;
                case 140:
                    if (!caps.contains(GLCaps.GLSL140)) return false;
                case 150:
                    if (!caps.contains(GLCaps.GLSL150)) return false;
                case 330:
                    if (!caps.contains(GLCaps.GLSL330)) return false;
                default:
                    return false;
            }
        }
