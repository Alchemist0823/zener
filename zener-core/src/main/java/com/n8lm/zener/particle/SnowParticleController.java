...
public class SnowParticleController implements ParticleController {
	...
	static final ColorRGBA color1 = new ColorRGBA(1, 1, 1, 0.8f);
	static final ColorRGBA color2 = new ColorRGBA(1, 1, 1, 0.4f);
	...
	public Particle setNewParticle(Particle p) {
		p.position.set((float) (Math.random() * 20), (float) (Math.random() * 20), 10);
		p.velocity.set((float) (Math.random() * 2 - 1), (float) (Math.random() * 2 - 1), (float) (Math.random() * 1 - 2.5));
