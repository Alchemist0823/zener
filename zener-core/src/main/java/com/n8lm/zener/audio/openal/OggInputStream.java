...
public class OggInputStream extends InputStream implements AudioInputStream {
  private final Logger log = Logger.getLogger(OggInputStream.class.getName());
   ...
   */
  @Nonnull
  private final StreamState streamState = new StreamState(); // take physical pages,
   ...
   */
  public OggInputStream(@Nonnull InputStream input) throws IOException {
    this.input = input;
  ...
  private boolean getPageAndPacket() {
    ...
    try {
      ...
      log.warning("Failure reading in vorbis: " + e.getMessage());
      endOfStream = true;
      return false;
    }
    ...
    if (syncState.pageout(page) != 1) {
      ...
      log.warning("Input does not appear to be an Ogg bitstream.");
      endOfStream = true;
      return false;
    }
    ...
    if (streamState.pagein(page) < 0) {
      ...
      log.warning("Error reading first page of Ogg bitstream data.");
      endOfStream = true;
      return false;
    }
    ...
    if (streamState.packetout(packet) != 1) {
      ...
      log.warning("Error reading initial header packet.");
      endOfStream = true;
      return false;
    }
    ...
    if (oggInfo.synthesis_headerin(comment, packet) < 0) {
      ...
      log.warning("This Ogg bitstream does not contain Vorbis audio data.");
      endOfStream = true;
      return false;
    }
    ...
    while (i < 2) {
        ...
        if (result == 1) {
          ...
          while (i < 2) {
            ...
            if (result == -1) {
              ...
              log.warning("Corrupt secondary header.  Exiting.");
              endOfStream = true;
              return false;
            }
      ...
      try {
        ...
        log.warning("Failed to read Vorbis: " + e.getMessage());
        endOfStream = true;
        return false;
      }
      if (bytes == 0 && i < 2) {
        log.warning("End of file before finding all Vorbis headers!");
        endOfStream = true;
        return false;
      }
      syncState.wrote(bytes);
              ...
              if (result == -1) { // missing or corrupt data at this page position
                ...
                while ((samples = dspState.synthesis_pcmout(_pcm,
                  ...
                  if (bytesToWrite >= pcmBuffer.remaining()) {
                    log.warning("Read block from OGG that was too big to be buffered: " + bytesToWrite);
                  } else {
        ...
        if (!endOfBitStream) {
          ...
          if (index >= 0) {
            ...
            try {
              ...
              log.warning("Failure during vorbis decoding: " + e.getMessage());
              endOfStream = true;
              return;
            }
          } else {
