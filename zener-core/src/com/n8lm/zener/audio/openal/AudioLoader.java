...
public class AudioLoader {
  ...
  public static Audio getStreamingAudio(
      ...
      @Nonnull String ref, @Nonnull ResourceManager resourceManager) throws IOException {
    init();

    if (format.equals(OGG)) {
      return SoundStore.get().getOggStream(ref, resourceManager);
    }
    if (format.equals(MOD)) {
      return SoundStore.get().getMOD(resourceManager.getResourceAsStream(ref));
    }
    if (format.equals(XM)) {
      return SoundStore.get().getMOD(resourceManager.getResourceAsStream(ref));
    }
