...
public final class Matrix3f implements Cloneable, Serializable {
    ...
    public void absoluteLocal() {
        m00 = (float) Math.abs(m00);
        m01 = (float) Math.abs(m01);
        m02 = (float) Math.abs(m02);
        m10 = (float) Math.abs(m10);
        m11 = (float) Math.abs(m11);
        m12 = (float) Math.abs(m12);
        m20 = (float) Math.abs(m20);
        m21 = (float) Math.abs(m21);
        m22 = (float) Math.abs(m22);
    }
...
//        set(tmp, true);
    ...
    public Matrix3f invert(Matrix3f store) {
        ...
        if ((float) Math.abs(det) <= (float) MathUtil.FLT_EPSILON) {
            return store.zero();
        }
    ...
    public void fromStartEndVectors(Vector3f start, Vector3f end) {
        ...
        if (f > 1.0f - (float) MathUtil.ZERO_TOLERANCE) {
